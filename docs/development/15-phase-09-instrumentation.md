...
**5. Compound tool candidates (sequential pattern analysis)**

Joins `sessions.jsonl` (tool sequences) with `metrics.jsonl` (response sizes) to
score every frequently-occurring adjacent pair and triple on token cost:

```python
from collections import defaultdict, Counter

# Build per-call response size lookup: {(session_id, tool_name, call_index): bytes}
call_sizes = index_metrics_by_session(metrics)   # from metrics.jsonl

pair_stats = defaultdict(lambda: {"count": 0, "b_bytes": []})
for session in sessions:
    seq = session["tool_sequence"]
    for i in range(len(seq) - 1):
        a, b = seq[i], seq[i + 1]
        b_bytes = call_sizes.get((session["session_id"], b, i + 1), 0)
        pair_stats[(a, b)]["count"] += 1
        pair_stats[(a, b)]["b_bytes"].append(b_bytes)

# merge_score = frequency × avg response bytes of B (the call that would be eliminated)
scored = [
    (a, b, stats["count"], mean(stats["b_bytes"]),
     stats["count"] * mean(stats["b_bytes"]) / 4)   # /4 → token estimate
...
The merge score weights frequency by the response size of B — the call that would
be eliminated or folded into A's response. A high-frequency pair where B returns
50 bytes scores lower than a moderate-frequency pair where B returns 4 KB, correctly
deprioritising merges that save negligible context.

Report output:
```
Compound tool candidates (ranked by estimated token savings):

Pair                                   freq   avg B resp   merge score   action
...
The "action" column is generated by rule:
- B is a read tool and A is a write tool → "fold B into A's response"
- B is a read tool and A is also a read tool → "add optional flag to A to include B's data"
- A and B are always the first two calls in a session → "merge into session-start tool"
...
T9.4.8  Compound tool candidates table ranks pairs by merge_score descending
T9.4.9  merge_score = frequency × avg_b_bytes / 4 (verify against manual calculation
...
| Compound tool merge scores | Identifies tool catalog changes that reduce calls before Option D is needed; high-scoring pairs are also the first candidates to expose through the Option D subagent interface |
